      ...
      <div class="border border-primary-100 rounded p-4">
        ...
        <FieldArray name="typeTransportOneWay" v-slot="{ fields, push, remove }" class="">
          <fieldset class="!mt-0 InputGroup" v-for="(field, idx) in fields" :key="field.key">
            <div class="relative grid grid-cols-1 md:grid-cols-6 lg:grid-cols-12 gap-4 mt-2 py-6" :class="{ 'border-t border-dashed border-primary-100': idx > 0 }">
              <BaseSelect class="md:col-span-2" :nameModel="`typeTransportOneWay[${idx}].transportTypes`"
                :listItens="transportTypes" label="Tipo de transporte" />
              <BaseInput class="md:col-span-3" :name="`typeTransportOneWay[${idx}].company`" type="text" label="Empresa"
                :value="field.value.company" />
              <BaseInput class="md:col-span-2" :name="`typeTransportOneWay[${idx}].line`" type="text" label="Linha"
                :value="field.value.line" />
              <BaseInput class="md:col-span-2" :name="`typeTransportOneWay[${idx}].quantity`" type="number"
                label="Quantidade" :value="field.value.quantity" />
              <BaseInput class="md:col-span-2" :name="`typeTransportOneWay[${idx}].value`" type="text"
                v-maska="['R$ #,##', 'R$ ##,##', 'R$ ###,##', 'R$ #.###,##', 'R$ ##.###,##', 'R$ ###.###,##', 'R$ #.###.###,##']"
                label="Valor passagem" :value="field.value.value" />
              <div v-for="cardType in transportCardType" class="md:col-span-2">
                <BaseCheckbox :name="`typeTransportOneWay[${idx}].transportCardType`" type="radion" :label="cardType.name"
                  :value="cardType.id" />
              </div>
              <button class="absolute lg:top-1/2 lg:transform lg:-translate-y-1/2 right-0 text-left w-1 col-span-1 text-negative-400 font-bold hover:opacity-70"
      ...
      <div class="border border-primary-100 rounded p-4">
        ...
        <FieldArray name="typeTransportReturn" v-slot="{ fields, push, remove }" class="">
          <fieldset class="InputGroup" v-for="(field, idx) in fields" :key="field.key">
            <div class="relative grid grid-cols-1 md:grid-cols-6 lg:grid-cols-12 gap-4 mt-2 py-6" :class="{ 'border-t border-dashed border-primary-100': idx > 0 }">
              <BaseSelect class="md:col-span-2" :nameModel="`typeTransportReturn[${idx}].transportTypes`"
                :listItens="transportTypes" label="Tipo" />
              <BaseInput class="md:col-span-3" :name="`typeTransportReturn[${idx}].company`" type="text" label="Empresa"
                :value="field.value.company" />
              <BaseInput class="md:col-span-2" :name="`typeTransportReturn[${idx}].line`" type="text" label="Linha"
                :value="field.value.line" />
              <BaseInput class="md:col-span-2" :name="`typeTransportReturn[${idx}].quantity`" type="number"
                label="Quantidade" :value="field.value.quantity" />
              <BaseInput class="md:col-span-2" :name="`typeTransportReturn[${idx}].value`" type="text"
                v-maska="['R$ #,##', 'R$ ##,##', 'R$ ###,##', 'R$ #.###,##', 'R$ ##.###,##', 'R$ ###.###,##', 'R$ #.###.###,##']"
                label="Valor passagem" :value="field.value.value" />
              <div v-for="cardType in transportCardType" class="md:col-span-2">
