    ...
    <div class="space-y-4 border border-primary-100 rounded p-4 mt-2">
        ...
        <FieldArray name="typeTransportOneWay" v-slot="{ fields, push, remove }" class="">
          <fieldset class="InputGroup" v-for="(field, idx) in fields" :key="field.key">
            <div class="relative grid grid-cols-12 gap-4 mt-2">
              ...
              <BaseInput class="col-span-1" :name="`typeTransportOneWay[${idx}].quantity`" type="text" v-maska="'#*'" label="Quantidade" />
              <BaseInput class="col-span-2" :name="`typeTransportOneWay[${idx}].value`" type="text" v-maska="['#,##', '##,##', '###,##', '#.###,##', '##.###,##', '###.###,##', '#.###.###,##']" label="Valor passagem" />
      ...
      <div class="border border-primary-100 rounded p-4">
        ...
        <FieldArray name="typeTransportReturn" v-slot="{ fields, push, remove }" class="">
          <fieldset class="InputGroup" v-for="(field, idx) in fields" :key="field.key">
            <div class="relative grid grid-cols-12 gap-4 mt-2">
              <BaseSelect class="col-span-2" :nameModel="`typeTransportReturn[${idx}].type`" :listItens="transportTypes" label="Tipo" />
              ...
              <BaseInput class="col-span-1" :name="`typeTransportReturn[${idx}].quantity`" type="text" v-maska="'#*'" label="Quantidade" />
              <BaseInput class="col-span-2" :name="`typeTransportReturn[${idx}].value`" type="text" v-maska="['#,##', '##,##', '###,##', '#.###,##', '##.###,##', '###.###,##', '#.###.###,##']" label="Valor passagem" />
              <button class="absolute right-0 top-2.5 col-span-1 text-negative-400 font-bold hover:opacity-70"
                type="button" @click="remove(idx)">X</button>
            </div>
          </fieldset>
